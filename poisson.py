...
def get_fano_factor1(spike_train, big_t, window_size):
    spike_count_list = []
    for i in np.arange(0, big_t, window_size*ms):
        start = i
        end = i+window_size*ms
        spikes_for_interval = 0
        for spike in spike_train:
            if spike > start and spike < end:
                spikes_for_interval+=1
        spike_count_list.append(spikes_for_interval)
        # print("start =",start," end=",end," spikes=",spikes_for_interval)
